...
void setup() {
  ...
  if (EEPROM.read(address + 100) != float(233)){ //检测是否是第一次刷入 无意义数值
    ...
    EEPROM.put(address + 40, float(300)); //初始关亮度
    EEPROM.put(address + 50, float(550)); //初始开亮度
...
String urlDecode(String input) {
  ...
  s.replace("%28", "(");s.replace("%29", ")");s.replace("%30", "*");
  s.replace("%31", "+");s.replace("%2C", ",");s.replace("%2E", ".");
  s.replace("%2F", "/");s.replace("%2C", ",");s.replace("%3A", ":");
  s.replace("%3A", ";");s.replace("%3C", "<");s.replace("%3D", "=");
  s.replace("%3E", ">");s.replace("%3F", "?");s.replace("%40", "@");
  s.replace("%5B", "[");s.replace("%5C", "\\");s.replace("%5D", "]");
  s.replace("%5E", "^");s.replace("%5F", "-");s.replace("%60", "`");
  return s;
}
